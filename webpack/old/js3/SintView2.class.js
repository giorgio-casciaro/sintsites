...
class SintView {
  ...
  renderDom () {
    ...
    this.log('renderDom RENDER', tempDom)
    ...
    if (!this.domPlaceholdersHtml || this.domPlaceholdersHtml !== renderedString) {
      ...
      this.log('renderDom CREATE DOM', tempDomPlaceholders.innerHTML)

      // domPlaceholders DIFF
      // if (this.domPlaceholders) this.domReplaceNodeContentsAndAttributes(tempDomPlaceholders, this.domPlaceholders)
      // else this.domPlaceholders = tempDomPlaceholders
      // this.log('renderDom domPlaceholders DIFF', tempDomPlaceholders.innerHTML)

      // var tempDom = this.domPlaceholders.clone()
      // SUBVIEWS PLACEHOLDERS SUBSTITUTION
      if (Object.keys(this.trackedViewsData).length) {
        let domPlaceholdersTags = this.domGetByTag('sintview', tempDomPlaceholders)
      ...
      var tempDom = tempDomPlaceholders

      if (this.dom) this.domReplaceNodeContentsAndAttributes(tempDom, this.dom)
      else this.dom = tempDom
      this.dom.sintView = this
      console.log('  this.dom.sintView', this.dom.sintView, this.dom)
    }

    this.log('renderDom 2', tempDom.innerHTML)
    return this.dom
  }
}
