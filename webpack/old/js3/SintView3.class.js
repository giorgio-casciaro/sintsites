...
class SintView {
  ...
  domReplaceNodeContentsAndAttributes (from, to) {
    ...
    dd.filterOuterDiff = (t1, t2, diffs) => {
      console.log('filterOuterDiff', this, { t1, t2, diffs })
      if (t1.attributes && t1.attributes.trackid && !diffSkipTrackIdTag) {
        // will not diff childNodes
        t1.innerDone = true
        t2.innerDone = true
      }
      diffSkipTrackIdTag = false
    }
    var diff = dd.diff(to, from)
    this.log('domReplaceNodeContentsAndAttributes', {
      from: from.innerHTML,
      to: to.innerHTML,
      diff
    })
    let result = dd.apply(to, diff)
    this.log('domReplaceNodeContentsAndAttributes after', {
      result,
      from: from.innerHTML,
      to: to.innerHTML,
      diff
  ...
  renderDom (setAsMainDom = true) {
    ...
    if (!this.domPlaceholdersHtml || this.domPlaceholdersHtml !== renderedString) {
      ...
      if (Object.keys(this.trackedViewsData).length) {
        ...
        for (let i = 0; i < domPlaceholdersTags.length; i++) {
          ...
          if (this.trackedViews[trackid]) {
            ...
            domPlaceholdersTags[i].parentNode.replaceChild(
              this.trackedViews[trackid].dom,
              domPlaceholdersTags[i]
            )
          } else {
            var trackedViewData = this.trackedViewsData[trackid]
            let subView = new SintView(
              trackedViewData[0],
              trackedViewData[1],
              this
            )
      ...
      if (setAsMainDom) {
        if (this.dom) {
          this.domReplaceNodeContentsAndAttributes(returnDom, this.dom)
