(function(root, factory) {
    ...
    if (el.nodeName !== "#text" && el.nodeName !== "#comment") {
      ...
      if (el.attributes) {
        if (el.attributes["class"]) {
          output.push(
            el.nodeName + "." + el.attributes["class"].replace(/ /g, ".")
          );
        }
        if (el.attributes.id) {
  ...
  var isEqual = function(e1, e2) {
    ...
    if (e1.attributes) {
      ...
      if (
        !e1Attributes.every(function(attribute) {
    ...
    if (e1.childNodes) {
      ...
      if (
        !e1.childNodes.every(function(childNode, index) {
  ...
  var roughlyEqual = function(
    ...
    if (e1.attributes && e2.attributes) {
      ...
      if (
        ...
        var classDescriptor =
          e1.nodeName + "." + e1.attributes["class"].replace(/ /g, ".");
  ...
  var markSubTrees = function(oldTree, newTree) {
    ...
    while (subset) {
      ...
      if (subset) {
        subsets.push(subset);
        subsetArray = Array.apply(null, new Array(subset.length)).map(
          returnIndex
        );
  ...
  diffDOM.prototype = {
    ...
    findOuterDiff: function(t1, t2, route) {
      ...
      if (
        ...
        t1.childNodes &&
        t2.childNodes &&
        t1.childNodes.length > this.maxChildCount &&
        t2.childNodes.length > this.maxChildCount
      ) {
        var childNodesLength =
            t1.childNodes.length < t2.childNodes.length
              ? t1.childNodes.length
              : t2.childNodes.length,
          childDiffCount = 0,
          j = 0;
        while (
      ...
      for (
        ...
        if (gaps1[index2] === true) {
          ...
          if (node.nodeName === "#text") {
            if (
              t2.childNodes[index2].nodeName === "#text" &&
              node.data !== t2.childNodes[index2].data
            ) {
              testI = index1;
              while (
      ...
      switch (diff[this._const.action]) {
        ...
        case this._const.relocateGroup:
          nodeArray = node.childNodes
            .splice(diff[this._const.from], diff.groupLength)
            .reverse();
          length = nodeArray.length;
          for (i = 0; i < length; i++) {
          ...
          if (node.subsets) {
            node.subsets.forEach(function(map) {
              if (
                diff[t._const.from] < diff[t._const.to] &&
                map.oldValue <= diff[t._const.to] &&
                map.oldValue > diff[t._const.from]
              ) {
      ...
      switch (diff[this._const.action]) {
        ...
        case this._const.modifyTextElement:
          ...
          this.textDiff(
            node,
            node.data,
            diff[this._const.oldValue],
            diff[this._const.newValue]
          );
          break;
        case this._const.modifyValue:
        ...
        case this._const.replaceElement:
          ...
          break;
        case this._const.relocateGroup:
          nodeArray = Array.apply(null, new Array(diff.groupLength)).map(
            function() {
