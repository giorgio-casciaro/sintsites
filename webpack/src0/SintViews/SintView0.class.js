...
export default class SintView {
  constructor (template, params, binds, parentView) {
  ...
  initParam (paramName) {
    // if (this.params[paramName] && typeof this.params[paramName] === 'function') {
    //   this.params[paramName] = this.params[paramName].call(this)
    // }
    if (this.params[paramName] && this.params[paramName].addBinding) {
  ...
  async viewRenderWithBinds (template, params, asDom, onInit) {
  ...
  async viewRender (template, params, asDom, onInit) {
    // this.log('viewRender', template, params, this.trackViews, this)
    if (typeof template === 'string')template = await this.getTemplate(template)
    var subView = new SintView(template, params, this)
    subView.init()
