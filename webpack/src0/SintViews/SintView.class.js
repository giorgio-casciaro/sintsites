import SintBinding from './SintBinding.class'
...
export default class SintView {
  ...
  initBind (bindName, bindData) {
    this.params[bindName] = new SintBinding(bindData[0], bindData[1])
    // this.log('this.params[bindName]', bindData, this.params[bindName])
    this.params[bindName].addBinding(
      this.params,
      bindName,
      () => { this.usedParams[bindName] = true },
      () => { if (this.usedParams[bindName]) this.domUpdate(bindName) },
      this
    )
  }
  initParam (paramName) {
