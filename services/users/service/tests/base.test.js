...
var startTest = async function (netClient) {
  ...
  async function setContext (contextData) {
    ...
    for (i in contextData.entities) {
      var obj = Object.assign({ id: 'testDash_11111111-1111-1111-1111-111111111111', VIEW_META: { created: Date.now(), updated: Date.now(), is_test: true } }, contextData.entities[i])
  ...
  test = await apiCall('read', { id: '11111111-1111-1111-1111-111111111111' }, { is_test: true })
  mainTest.testRaw('readPublic', test, (data) => !data.error && data.success && data.partial && data.view.publicName && !data.email)
  ...
  test = await apiCallInternal('getEvents', { type: 'log', service: 'MsUsersTest' }, { is_test: true, token: context.tokens['11111111-1111-1111-1111-111111111111'] }, true)
  var eventsCounter = 0
  test.on('data', () => { eventsCounter++ }).on('error', (data) => log('stream error', data)).on('end', (data) => log('stream end', data))
  ...
  await test.destroy()
  await new Promise((resolve) => setTimeout(resolve, 5000))
  mainTest.testRaw('GET EVENTS', eventsCounter, (data) => data > 9)
  await context.destroy()
  ...
  context = await setContext({ data: { }, users: { '11111111-1111-1111-1111-111111111111': { permissions: ['dashboardsCreate'] } }, entities: [] })
  test = await apiCallInternal('getEvents', { type: 'log', service: 'MsUsersTest' }, { is_test: true, token: context.tokens['11111111-1111-1111-1111-111111111111'] }, true)
  test2 = await apiCallInternal('getEvents', { type: 'log', service: 'MsUsersTest' }, { is_test: true, token: context.tokens['11111111-1111-1111-1111-111111111111'] }, true)
  var eventsBCounter = 0
  test.on('data', () => { log('test1', eventsBCounter); eventsBCounter++ }).on('error', (data) => log('stream error', data)).on('end', (data) => log('stream end', data))
  test2.on('data', () => { log('test2', eventsBCounter); eventsBCounter++ }).on('error', (data) => log('stream error', data)).on('end', (data) => log('stream end', data))
  //
  ...
  await test.destroy()
  await test2.destroy()
  mainTest.testRaw('GET EVENTS', eventsBCounter, (data) => data > 9)
  // await context.destroy()
  ...
  test = await apiCall('read', { id: '11111111-1111-1111-1111-111111111111' }, { is_test: true, token: context.tokens['22222222-2222-2222-2222-222222222222'] })
