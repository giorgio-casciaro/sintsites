...
class LiveSessionClass extends require('events') {
    ...
    allSessions.mainServiceEventsBus.on('remoteEvent', async (eventInfo) => {
      ...
      if (eventName === 'mutation') {
        // if (eventData.mutation === 'create') {
          ...
          //           this.unmonitorView(viewId, request.params.queryParams.fields, true)
          //         }
          //       }
          //       for (let viewId of response.results) {
          //         if (query.viewIds.indexOf(viewId) < 0) {
          //           this.monitorView(viewId, request.params.queryParams.fields, true)
          //         }
          //       }
          ...
          //     //   this.monitorView(result.id, request.params.queryParams.fields)
  ...
  async monitorView (queryId, id, fields, load = false) {
    ...
    if (!view) {
      ...
      fields.forEach(field => {
        ...
        view.fieldQueries[field].push(queryId)
      })
    }
    if (!view.monitor) {
      view.monitor = true
  ...
  async updateQuery (query) {
    ...
    if (response.ids) {
      if (response.ids.toString() !== query.viewIds.toString()) {
        ...
        for (let viewId of query.viewIds) if (response.ids.indexOf(viewId) < 0) this.unmonitorView(query.id, viewId, query.fields)
        for (let viewId of response.ids) if (query.viewIds.indexOf(viewId) < 0) await this.monitorView(query.id, viewId, query.fields, true)
  ...
  async addedRequest (request) {
    ...
    if (!this.queries[request.queryId].monitor) {
      this.queries[request.queryId].monitor = true
  ...
  async garbageCollection (maxViews, maxQueries) {
    ...
    log('LiveSessionClass garbageCollection', { queriesArray })
...
var serviceMethods = {
  ...
  liveSession: {
    ...
    request: { lastUpdate: { type: 'number' }, maxViews: { type: 'number' }, maxQueries: { type: 'number' } },
    response: { },
    exec: async function (reqData, meta = { directCall: true, is_test: false }, getStream = null) {
      /*
