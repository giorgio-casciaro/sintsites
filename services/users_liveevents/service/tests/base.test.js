...
var startTest = async function (netClient) {
  ...
  var getEvents = function () {
    ...
    es.addEventListener('resendRequests', async function (e) {
      console.log('resendRequests', sessionResults.requests)
      sessionResults.resendRequestsCount++
      for (let requestId in sessionResults.requests) {
        let request = sessionResults.requests[requestId]
        ...
        console.log('resendRequests sendRequest', sendRequestResponse)
      }
    })
    es.sendRequest = async function (query, queryParams) {
      ...
      console.log('sendRequest', sendRequestResponse)
      let id = sendRequestResponse[0].data.requestId
      let queryId = sendRequestResponse[0].data.queryId
      sessionResults.requests[id] = { id, queryId, query, queryParams, data: sendRequestResponse[0].data }
      return id
    }
    es.removeRequest = async function (requestId) {
      let removeRequestResponse = await apiCallInternal('liveSessionCommands', { commands: [{ command: 'removeRequest', params: { requestId } }], sessionId: sessionId, listenerId: listenerId }, { is_test: true, token: context.tokens.userAdminTest })
      delete sessionResults.requests[requestId]
      return removeRequestResponse[0].data.removedQueryId
    }
  ...
  // mainTest.consoleResume()
  await new Promise((resolve) => setTimeout(resolve, 500))
  var lastUpdatedReqId = await es.sendRequest('lastUpdated', { fields: ['publicName'] })
  var queryListId = sessionResults.requests[lastUpdatedReqId].queryId
  await new Promise((resolve) => setTimeout(resolve, 1000))
  ...
  var lastUpdatedRemoveRequestQueryId = await es.removeRequest(lastUpdatedReqId)

  var lastUpdatedPage2ReqId = await es.sendRequest('lastUpdated', { fields: ['publicName'], from: 20, to: 30 })
  await new Promise((resolve) => setTimeout(resolve, 1000))
  await es.removeRequest(lastUpdatedPage2ReqId)

  var lastUpdatedPage3ReqId = await es.sendRequest('lastUpdated', { fields: ['publicName'], from: 30, to: 40 })
  await new Promise((resolve) => setTimeout(resolve, 1000))
  await es.removeRequest(lastUpdatedPage3ReqId)

  await new Promise((resolve) => setTimeout(resolve, 1000))

  mainTest.testRaw('LIVE SESSION QUERY REMOvED AFTER GARBAGE COLLECTION (3 queries max)', sessionResults, (data) => !data.queries[lastUpdatedRemoveRequestQueryId])
  // mainTest.consoleResume()
  await new Promise((resolve) => setTimeout(resolve, 1000))
  ...
  es = getEvents()
  await new Promise((resolve) => setTimeout(resolve, 1000))
  mainTest.testRaw('LIVE SESSION UPDATE VIEW AFTER long disconnection (5 sec on keepAlive 3 sec)', sessionResults, (data) => data.views['11111111-1111-1111-1111-111111111111'].publicName === 'NewPublicName2' && data.resendRequestsCount === 1)
  mainTest.consoleResume()

  var lastUpdatedReqId2 = await es.sendRequest('lastUpdated', { fields: ['publicName'] })
  var queryListId2 = sessionResults.requests[lastUpdatedReqId2].queryId
  await new Promise((resolve) => setTimeout(resolve, 1000))
  es.close()
  await new Promise((resolve) => setTimeout(resolve, 3000))
  var createUserResponse2 = await createUser()
  await new Promise((resolve) => setTimeout(resolve, 3000))
  es = getEvents()
  await new Promise((resolve) => setTimeout(resolve, 1000))
  mainTest.testRaw('LIVE SESSION  UPDATE QUERY AFTER long disconnection (5 sec on keepAlive 3 sec)', sessionResults, (data) => data.queries[queryListId2].indexOf(createUserResponse2.id) > -1)
  mainTest.consoleResume()
